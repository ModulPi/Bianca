...
from backend.infrastructure.checkpoint.saver import checkpointer_context
from backend.config import Settings, get_settings
from backend.application.degradation import get_effective_execution_mode
from backend.application.graph.analysis_agent import apply_analysis_to_state, run_analysis_agent
from backend.application.graph.execute_agent import run_execute_agent
from backend.application.graph.merge_signals import merge_signals
from backend.application.graph.orchestrator import orchestrator_node
from backend.application.graph.pending_agent import run_pending_agent
from backend.application.graph.risk_agent import run_risk_agent
from backend.application.graph.state import TradeState
from backend.application.graph.strategy_agent import strategy_node
from backend.domain.markets.registry import get_market_adapter
from backend.infrastructure.storage.repository import TradeRepository
...
async def fetch_market_node(state: TradeState) -> TradeState:
    ...
    if settings.trade_market == "crypto":
        from backend.infrastructure.market.klines import persist_recent_klines
