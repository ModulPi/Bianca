...
from backend.application.degradation import clear_degradation, get_effective_execution_mode, record_tick_failure, record_tick_success
from backend.application.graph.supervisor import run_agent_tick
from backend.domain.markets.registry import get_market_adapter
from backend.infrastructure.storage.repository import AgentConfigRepository
from backend.application.summary.aggregator import close_session
...
class AgentRunner:
    ...
    async def start(self) -> None:
        ...
        from backend.application.metrics import set_agent_running

        set_agent_running(True)
        from backend.application.validation.paper_gate import assert_demo_mode_for_trading, ensure_validation_running
        ...
        if self._snapshot.session_id and self._snapshot.session_started_at:
            from backend.infrastructure.cache.redis_client import set_active_session
        ...
        if settings.market_stream_enabled:
            from backend.infrastructure.market.stream_manager import get_stream_manager
    ...
    async def stop(self) -> None:
        ...
        if self._snapshot.session_id:
            from backend.infrastructure.cache.redis_client import clear_active_session
    ...
    async def get_snapshot(self) -> RunnerSnapshot:
        ...
        from backend.application.degradation import is_degraded
    ...
    async def _run_one_tick(self, settings: Settings, symbol: str) -> None:
        ...
        try:
            ...
            from backend.application.metrics import record_agent_tick
