...
from backend.infrastructure.storage.constants import (
    DEFAULT_AGENT_STRATEGY_ID,
    DEFAULT_AGENT_STRATEGY_NAME,
)
from backend.infrastructure.storage.database import get_session_factory, schema_mode
...
class KlineRepository:
    ...
    async def list_recent(
        ...
        *,
        ...
        """按 symbol/interval 取最近 K 线（时间降序）。非 MVP 栈返回空。"""
        if schema_mode() != "mvp":
            return []
        stmt = text(
            """
            SELECT time, symbol, interval, open, high, low, close, volume, trades
            FROM klines
            WHERE symbol = :symbol AND interval = :interval
            ORDER BY time DESC
            LIMIT :limit
        ...
        async with factory() as db:
            result = await db.execute(
                stmt, {"symbol": symbol, "interval": interval, "limit": limit}
            )
            rows = result.mappings().all()
        out: list[dict] = []
        for row in rows:
            ...
            if isinstance(ts, datetime):
                ts_s = (ts.astimezone(UTC) if ts.tzinfo else ts.replace(tzinfo=UTC)).isoformat()
            else:
                ts_s = str(ts)
            out.append(
                {
    ...
    async def insert_candles(
        ...
        *,
        ...
        """写入 OHLCV 元组列表 (ts_ms, o, h, l, c, v[, trades])。"""
        if schema_mode() != "mvp" or not candles:
            return 0
        ...
        bars = [
            SimpleNamespace(
                ...
                trades=int(candle[6]) if len(candle) > 6 else 0,
            )
            for candle in candles
        ]
        return await self.insert_bars(bars)
