...
from backend.domain.strategy.engine import default_params, run_strategy_tick
from backend.domain.strategy.runner import get_strategy_runner
...
async def start_strategy(strategy_id: str) -> StrategyItem:
    ...
    if row is None:
        raise HTTPException(status_code=404, detail="Strategy not found")
    from backend.application.validation.paper_gate import ensure_validation_running
