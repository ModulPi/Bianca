...
> 版本：v0.4 | 日期：2026-08-04 | 阶段：MVP 已交付 | 状态：Agent 自主交易 + 运维看板 ✅
...
Bianca 是一个 **24×7 自主交易 Agent 引擎**（crypto 优先），基于 LangGraph 编排 LLM 决策、风控与执行；配套 **Agent 运维看板** 供运维者监控仓位、收益、Token 与降级人工介入。**不是** C 端交易平台或策略商城。

**PoC 聚焦：** 在币安 **Demo 现货** 环境上，由 **LLM 自主决策** 完成至少一次 **买入 → 卖出** 闭环。

**MVP 及当前：** 多 Worker 24×7 运行 · 完整风控 · 半自动降级 · 运维看板 · 合约 Demo 钩子 · 模拟→实盘门禁。

### 1.2 目标用户

| 用户画像 | 特征 | 核心诉求 |
|----------|------|----------|
| Agent 运维者 | 部署/监控自主 Agent | 24×7 可观测、异常可介入 |
| 开发者 | 本地 PoC/MVP 验证 | 闭环可验收、决策可回放 |
| （后期）跟单用户 | 跟随信号 | P2 范围，当前不做 |
...
| 项 | 范围 |
|----|------|
| **界面** | **Agent 运维看板**（监控·仓位·收益·Token·降级确认）；见 [Agent运维看板设计](./outline-design/架构设计/Agent运维看板设计-Bianca.md) |
| **执行模式** | 增加 **半自动**（AI 建议 → 用户确认 → 执行） |
| **策略** | 策略模板引擎（后端实验保留，**无产品 UI**） |
| **品种** | 现货 + U 本位合约 + 币本位合约 |
| **风控** | 完整 8 条规则 + 熔断状态机 |
...
| **汇总管理** | 会话级 Token/成交/盈亏一键汇总；历史会话快照与 Web 仪表盘 |
...
| 功能 | 描述 |
|------|------|
| **Agent 运维看板** | 启停/Worker/行情/仓位/PnL/Token/降级确认；见 [设计文档](./outline-design/架构设计/Agent运维看板设计-Bianca.md) |
| 策略模板引擎 | 网格、DCA、趋势（**后端实验，非产品入口**） |
| 半自动模式 | 连续失败降级 → 看板待确认 → 风控 → 执行 |
| 完整风控 | 8 条核心规则 + symbol 白名单 + 熔断器 |
| 合约 API | U 本位 + 币本位 Demo 钩子 |
| 告警通知 | Telegram Bot / 邮件 |
| 模拟→实盘门禁 | 模拟验证达标检查（看板只读展示） |
| **汇总管理** | Summary API + 会话快照 + CSV 导出 |
...
```
LLM/策略 产生信号 → 自动降级 semi_auto → 看板 WS 推送待确认
→ 用户 confirm/reject → Risk Agent → Execute Agent → 日志
```
...
**MVP 扩展：** + React **运维看板** + PostgreSQL/TimescaleDB + Redis + 完整风控 + Summary API。
...
| D8 | PoC 界面 | 无前端；MVP 做 **Agent 运维看板**（非交易控制台） | 2026-07-31 |
| D13 | 产品定位 | Agent 自主交易引擎 + 运维看板；砍掉策略商城/K 线/密钥 Web CRUD | 2026-08-04 |
