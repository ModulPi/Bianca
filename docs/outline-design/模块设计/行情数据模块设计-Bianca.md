...
> 状态：**阶段一代码已落地并实测**（§6.1）；阶段一验收 9 项中 5 项通过、2 项部分验证、2 项未验（§8），另有 3 项因"数据层独立"要求新增（§8 追加）。v0.4：全量历史回补完成（4,778,552 行 / 809.2 MB / 覆盖率 99.8197%），修回补并发统计丢失更新，容量估算按实测行宽修正。**v0.5：ADR-010 标记为与产品要求冲突，新增提案 ADR-017「数据面作为独立运行单元」；上位架构文档的缺口已定位（§9.2）**
...
- **当前实测缺口（四条，均须先解）:**
  1. **无独立入口** —— `agent/market/` 无 `__main__`，`pyproject.toml` 无 `console_scripts`，数据面无法脱离 API 进程启动；
  2. **无进程守护** —— 无计划任务 / 服务 / Run 键（四种自启机制均已实测确认全空），重启后无人拉起；
  3. **部署形态是单服务** —— 架构文档 §2 容器图与系统设计文档 §6 `docker-compose.yml` 都只有 `api` 一个服务，没有数据面进程；
  4. **观测面耦合** —— `/api/v1/market/status`、`/api/v1/market/backfill` 挂在 API 的 router 上，数据面独立后这些端点的归属需要重新设计。
...
- [ ] **数据面可脱离 API 进程独立运行** —— 现状**不具备**：`agent/market/` 无独立入口，无 `console_scripts`，只能靠 FastAPI lifespan 拉起（ADR-017 缺口 1）
- [ ] **进程守护：关机/崩溃后自动恢复** —— 现状**不具备**：四种自启机制（计划任务 / 启动文件夹 / Run 键 / 服务）实测全空，重启后无人拉起（ADR-017 缺口 2）
- [ ] **实测确认采集器进程的稳定归属** —— 已发现它挂在上一轮 agent 会话的 shell 链下（祖先进程已消失），**关机必死、关 IDE 是否连带被杀未测**。在无人值守达标前，此项必须先解
