...
> 状态：**阶段一代码已落地并实测**（§6.1）；阶段一验收 9 项中 5 项通过、2 项部分验证、2 项未验（§8），另有 3 项因"数据层独立"要求新增（§8 追加，其中 1 项已达成）。v0.4：全量历史回补完成（4,778,552 行 / 809.2 MB / 覆盖率 99.8197%），修回补并发统计丢失更新，容量估算按实测行宽修正。v0.5：ADR-010 标记为与产品要求冲突，新增提案 ADR-017「数据面作为独立运行单元」；上位架构文档的缺口已定位（§9.2）。**v0.6：ADR-017 缺口 1 落地 —— 数据面有了独立入口（`python -m agent.market`）与单实例锁，10 项实测全 PASS；同时实测确认跨进程观测不可用（缺口 4 不只是"端点归属"，是"读不到"）**
...
- **状态:** 🔶 **部分实施中（2026-09-23）** | **日期:** 2026-09-23
  - 缺口 1（独立入口）**已解**：`python -m agent.market` / `[project.scripts] bianca-market`，附单实例锁 `agent/market/lock.py` 与验收脚本 `scripts/verify_market_standalone.py`（10 项全 PASS）。供数接口、进程守护、观测面解耦均未动。
...
- **当前实测缺口（四条，均须先解）:**
  ...
  2. **无进程守护** —— 无计划任务 / 服务 / Run 键（四种自启机制均已实测确认全空），重启后无人拉起；
...
- [ ] **进程守护：关机/崩溃后自动恢复** —— 现状**不具备**：四种自启机制（计划任务 / 启动文件夹 / Run 键 / 服务）实测全空，重启后无人拉起（ADR-017 缺口 2）
- [~] **实测确认采集器进程的稳定归属** —— **仍未解**。独立入口已具备（进程归属可以由我们自己指定，不再必然挂在启动它的 shell 链下），但线上那个采集器仍是改动前启动的、归属未变的进程。另新增一条已知窗口：**老进程不持单实例锁**（锁是这次才引入的），所以它可以和一个新采集器同时跑而互不报警 —— 重启它才能闭合
...
| 文档大版本号 | ✅ 本设计文档 **v0.5**：v0.3 补 §2.7 可观测字段与健康判定、§6.1 实现期实测、§8 阶段一验收实况；v0.4 补全量回补实测（§6.1 #26-30）、并发统计丢失更新（纠正 #3）、`backfill_history()` 补洞边界 + 缺口归因的外部交叉验证（纠正 #4）、§4 容量估算按实测行宽修正；v0.5 标记 ADR-010 与产品要求冲突、新增提案 ADR-017、定位上位文档缺口（§9.2）；v0.6 落地 ADR-017 缺口 1（独立入口 + 单实例锁）并实测跨进程观测不可用 |
...
| 10 | **数据面独立运行单元**（重开 ADR-010） | 架构、部署、无人值守 | ⬜ **待定（提案 ADR-017）**。逻辑分离已达成（ADR-007，代码依赖方向已实测干净），缺的是**运行期/部署期**独立：无独立入口、无进程守护、单服务部署、观测端点耦合 API。核心待决是**供数接口形态** —— 同进程函数调用 vs 共享库只读直读 vs IPC/HTTP，倾向前者中的"共享库只读" |
