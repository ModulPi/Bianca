...
| 32 | **真实故障：断流 13.78h + 重启自动恢复 + 退避首次触发**（用户断网 + 重启机器） | ✅ 比计划中的断网测试更强的演示。**时间线**（本地时）：01:43 最后一根正常 bar → 关机 → **15:24:58** 计划任务在开机约 1 分钟后拉起采集器（`pid 8328`，父进程 `svchost`=任务计划服务）→ 15:25:13–15:31:37 网络被拒（6.4 分钟）→ **15:31:37** WS 自动重连、`found 1 gap(s) covering 828 bars`、`gap check filled 828 bars` → 15:35:11 `connected=yes lag=11s flowing=yes`。**退避第一次被真实触发**：1s→2s→4s→8s→16s→32s→60s 封顶（连续 5 次 60s），全程无人工干预。断流期间状态行：`connected=no lag=49631s bars24h=612 gaps24h=0 flowing=no owner=self reconnects=10 last_error='ConnectionRefusedError: [WinError 1225] 远程计算机拒绝网络连接。'` —— 新判定在真实故障里说出了正确的话。**数据无洞（独立查库复核）**：全表 4,779,586 行、`distinct==count` 为真、近 24h 1441 行、**非连续跳点 0 处**。守护复验（临时库）三项仍全 PASS，含硬杀后心跳复活 `12124 → 10188`（`rows 1000 → 2882`）。**另两个副产品**：① 真任务 `LastResult=0x800710E0`（`IgnoreNew` 挡回心跳的返回值，与 `State=Running` 吻合）**非零但机制正常** —— 只看这个码会误判，正是 §6.1 #22 那条的再次印证；② 暴露 `gap_count_24h` 的锚点缺陷（断流期间报 0），记入 §10 开放问题 #14 |
...
- [x] 断网 → 恢复，采集自动续上且数据无洞 —— ✅ **2026-09-24 实测通过**（§6.1 #32）。退避**第一次被真实触发**（1s→2s→4s→8s→16s→32s→60s 封顶），WS 自动重连、无需任何干预，`gap check filled 828 bars` 填平 13.78 小时空洞；**独立查库复核**近 24h 1441 行、非连续跳点 **0 处**。断流期间状态行 `flowing=no` + `last_error` 记录完整 —— 这是判定逻辑第一次面对真实退避重连
...
> 2026-09-24：**24h 连续运行已由用户明确推迟**（当前设备条件不支持连续开机 24h），**不计入阻塞项**；**断网恢复已实测通过**（§6.1 #32）。24h 这一项未完成前，阶段一按"数据底座尚未验收"对待 —— 这不是措辞问题：**未验证的验收项与验过的等价性只存在于纸面上**。
> 另注：`gap_count_24h` 在本次 13.78 小时断流期间报 **0**（同期 `bars_count_24h` 从 1439 塌到 612），**已知未修** —— 见 §10 开放问题 #14。它不影响 `/health` 的告警（那条走 `lag_seconds`，本次正常响起），但会误导只看 `/market/status` 的人。
...
| 文档大版本号 | ✅ 本设计文档 **v0.9**：v0.3 补 §2.7 可观测字段与健康判定、§6.1 实现期实测、§8 阶段一验收实况；v0.4 补全量回补实测（§6.1 #26-30）、并发统计丢失更新（纠正 #3）、`backfill_history()` 补洞边界 + 缺口归因的外部交叉验证（纠正 #4）、§4 容量估算按实测行宽修正；v0.5 标记 ADR-010 与产品要求冲突、新增提案 ADR-017、定位上位文档缺口（§9.2）；v0.6 落地 ADR-017 缺口 1（独立入口 + 单实例锁）并实测跨进程观测不可用；v0.7 落地 ADR-017 缺口 2（进程守护）并实测暴露计划任务两个静默陷阱（§6.1 纠正 #5）；v0.8 止住观测面的跨进程谎报（§2.7 分区重排 + 锁探针 + `gap_count_24h` 语义更正），实测风险 #1 的告警首次真的响（§6.1 #31、纠正 #6）；v0.9 记录一次真实故障（断流 13.78h + 重启自动恢复 + 退避首次触发，§6.1 #32），关闭"断网恢复"验收项，并把新暴露的 `gap_count_24h` 锚点缺陷记入开放问题 #14 |
...
| 14 | **`gap_count_24h` 的锚点错了 —— 真正断流时它读 0** | 观测可信度 | ⬜ **已知未修（2026-09-24 实测暴露）**。13.78 小时断流期间 `bars_count_24h` 从 1439 塌到 612，而 `gap_count_24h` 报 **0**。根因：`expected_24h = count_bars(day_ago, last_bar, interval)` **把量尺的右端锚在"库里最后一根 bar"上** —— 采集器一停，`expected` 与 `actual` 一起缩，差值恒为 0，于是这个字段在它唯一存在的理由（停流）面前失效。同一函数里正确的锚点已经算好了：`last_closed_bar_open(now_ms, interval)`（按 `now` 推"此刻本应已收盘的最后一根"）。换成它，那次读数应为 `1440 - 612 = 828` —— 正好是后来补回来的根数。**注意警报通道不受影响**：`/health` 走 `lag_seconds`，本次正常转 `error`；受影响的只是只看 `/market/status` 的人。**修法明确、改动很小，用户选择先推阶段二、此项留待后续** |
